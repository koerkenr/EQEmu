...
-- ============================================================================
-- This will FORCE AN ERROR if you haven't reviewed the QA output above.
...
SELECT
    CASE
        WHEN @continue_generation = 1 THEN '✓ Gate passed - continuing with generation'
        ELSE (SELECT 'ERROR: Set @continue_generation = 1 to proceed' FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'this_table_does_not_exist_abort_here')
    END AS gate_check;
...
SELECT
    CASE
        ...
        ELSE (SELECT CONCAT('FATAL: ', COUNT(*), ' T1 IDs already exist!') FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_NAME = 'collision_detected_aborting')
...
SELECT
    ...
    i.minstatus,
    i.Name,
    -- Define "is_real_gear" once for consistency (includes jewelry with ac=0 but has stats)
