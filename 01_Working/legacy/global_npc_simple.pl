...
#
# Simple approach: Roll for tiers at spawn, apply at death using AddLootTable modification
...
sub EVENT_SPAWN {
  ...
  # Get all base items from loot table that have tier variants
  my $sql = q{
    SELECT
  ...
  if (%tier_replacements) {
    ...
    quest::debug("TierLoot SPAWN: Stored " . scalar(keys %tier_replacements) . " tier replacements for NPC " . $npc->GetCleanName());
  }
}

sub EVENT_DEATH_COMPLETE {
  # This fires after loot is generated
  ...
  quest::debug("TierLoot DEATH_COMPLETE: Processing " . scalar(keys %replacements) . " replacements");
  
  # Add tier items directly to corpse
  foreach my $base_id (keys %replacements) {
    my $tier_id = $replacements{$base_id};
    
    # Try to add tier item
    if (defined &quest::addloot) {
      quest::addloot($tier_id, 0, 1, 1);
      quest::debug("TierLoot DEATH_COMPLETE: Added tier item $tier_id (base was $base_id)");
    }
  }
}
