-- ============================================================================
-- EQEmu Item Tiering System - T1 with SELECTIVE INJECTION
-- ============================================================================
-- Date: 2026-01-29
-- Strategy:
-- - Multiply existing stats (AC ×1.10, HP/Mana/Endur ×1.18, Stats ×1.10)
-- - Selectively inject 2-3 missing stats based on role (NOT uniform +1 all)
-- - NO premium stats at T1 (accuracy, attack, strikethrough, avoidance, stun resist)
-- - Use MOD(item_id, N) for pseudo-random but deterministic variety
-- - Respect class identity (no mana on melee, no attack on casters)
-- ============================================================================
...
WHERE slots > 0
  ...
  AND id < 300000  -- Only base items (below tier offsets)
  AND NOT EXISTS (
...
SELECT
    ...
    CASE
        WHEN MAX(id) < 300000 THEN '✓ Safe to use 300K+ offsets'
        ELSE '⚠ WARNING: Items exist in tier offset range!'
...
SELECT
    ...
    END AS result
FROM items
WHERE id >= 300000 AND id < 1000000;
...
SET @collision_count = (SELECT COUNT(*) FROM items i JOIN eligible_gear eg ON eg.id + 300000 = i.id);

-- This will cause a guaranteed error if collisions exist (division by zero)
SELECT IF(@collision_count = 0, '✓ No ID collisions for T1 gear (300K offset)', 1/0) AS collision_check;
...
SELECT
    i.id + 300000 AS id,
    i.minstatus,
    CONCAT(i.Name, ' ', CHAR(0x12), '(Enhanced)', CHAR(0x12)) AS Name,
    -- Stats: Multiply existing ×1.10, OR selectively inject based on role
    -- Use MOD(i.id, 3) for pseudo-random selection (gives 0,1,2)
    -- AGI: Melee DPS only, 2/3 chance
    ...
    -- AC: Tanks get +2, melee DPS get +1, casters get 0
    ...
    -- DEX: Melee DPS only, 2/3 chance (different MOD values than AGI for variety)
    ...
    -- INT: INT casters only, 2/3 chance
    ...
    -- WIS: WIS casters only, 2/3 chance
    ...
    -- Resists: Multiply existing, OR inject ONE random resist +2 (use MOD(i.id, 5) for 0-4)
    ...
    -- HP: Multiply existing ×1.18, OR inject based on role (tanks get more)
    GREATEST(FLOOR(COALESCE(i.hp,0) * 1.18),
        CASE
            ...
            ELSE 0
        END) AS hp,
    -- Mana: Multiply existing ×1.18, OR inject ONLY for casters (respect class identity)
    ...
    -- Endur: Multiply existing ×1.18, OR inject for tanks/melee only
    ...
    -- Weapon damage: +5% base damage (min +1), delay FROZEN
    CASE
        WHEN i.delay > 0 THEN GREATEST(i.damage + FLOOR(i.damage * 0.05), i.damage + 1)
        ELSE i.damage
    END AS damage,
    i.delay,
    ...
    i.filename, i.fvnodrop, i.clicklevel, i.icon, i.idfile,
    ...
    i.pendingloreflag, i.procrate, i.races, i.`range`, i.reclevel, i.recskill, i.reqlevel,
...
AND NOT EXISTS (
    SELECT 1 FROM items WHERE id = i.id + 300000
);
...
SELECT
    ...
    CASE
        WHEN COALESCE(i.bagwr,0) < 50 THEN LEAST(COALESCE(i.bagwr,0) + 10, 50)
        ELSE LEAST(COALESCE(i.bagwr,0) + 10, 90)
    END AS bagwr,
    i.bagsize,
    i.bagtype,
    -- Effects: unchanged
...
AND NOT EXISTS (SELECT 1 FROM items WHERE id = i.id + 300000);
...
JOIN items i ON eb.id = i.id
WHERE NOT EXISTS (SELECT 1 FROM item_tier_map WHERE base_item_id = i.id AND tier_code = 1)
AND EXISTS (SELECT 1 FROM items WHERE id = i.id + 300000);
